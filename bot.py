...
SUBMITTED_REGISTRY_FILE = DATA_DIR / "submitted_registry.json"
...
# admin_id -> sid когда админ выбрал отказ и бот ждёт причину
admin_pending_reject_reason: dict[int, int] = {}
...
def _norm_server_key(s: str) -> str:
    key = s.lower().strip()
    key = key.replace("ё", "е")
...
def _load_server_aliases() -> dict[str, str]:
    aliases: dict[str, str] = {}
    if not SERVERS_FILE.is_file():
        return aliases
    try:
        raw = SERVERS_FILE.read_text(encoding="utf-8")
    except OSError:
        return aliases
    ...
    for line in raw.splitlines():
        ...
        if not src or src.startswith("#"):
            continue
        if ":" in src:
            canonical, aliases_raw = src.split(":", 1)
            canonical = canonical.strip()
            alias_items = [x.strip() for x in aliases_raw.split(",") if x.strip()]
        else:
            canonical = src
            alias_items = []
        ...
        if not canonical:
            continue
        keys = [canonical, *alias_items]
        for candidate in keys:
            key = _norm_server_key(candidate)
            if key:
                aliases[key] = canonical
    return aliases


def parse_static_and_server(text: str) -> tuple[bool, str, str | None]:
    parts = re.split(r"\s+", text.strip(), maxsplit=1)
    if len(parts) < 2:
        return False, "Укажи и статик, и сервер (пример: 12345 La Puerta).", None
    static_part, server_part = parts[0], parts[1]
    if not re.fullmatch(r"\d{3,10}", static_part):
        return False, "Статик должен быть числом (пример: 12345).", None
    server_key = _norm_server_key(server_part)
    if not server_key:
        return False, "После статика укажи название сервера GTA5RP.", None
    server_name = SERVER_ALIASES.get(server_key)
    if not server_name:
        return (
            False,
            "Вы наверное ошиблись в вводе сервера. Проверь название сервера GTA5RP и отправь ещё раз.",
            None,
        )
    return True, f"{static_part} {server_name}", server_name
...
def append_submitted_registry(entry: dict) -> None:
    data = _load_json(SUBMITTED_REGISTRY_FILE, {"items": []})
    items = data.get("items")
    if not isinstance(items, list):
        items = []
    items.append(entry)
    data["items"] = items
    _save_json(SUBMITTED_REGISTRY_FILE, data)
...
def set_submission_reject_reason(sid: int, reason: str) -> bool:
    data = _load_submissions()
    changed = False
    for it in data.get("items", []):
        if int(it.get("id", 0)) == sid:
            it["reject_reason"] = reason.strip()
            changed = True
            break
    if changed:
        _save_submissions(data)
...
def set_submission_payment(sid: int, payment_status: str) -> bool:
    ...
    for it in data.get("items", []):
        if int(it.get("id", 0)) == sid:
            it["payment_status"] = payment_status
            changed = True
            break
    if changed:
        _save_submissions(data)
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in ADMIN_IDS and uid in admin_pending_reject_reason:
        await handle_admin_menu_text(update, context)
        return
    ...
    if st.get("stage") == "static":
        ...
        if not ok:
            ...
            return
        parsed_ok, parsed_or_err, _server_name = parse_static_and_server(text)
        if not parsed_ok:
            await msg.reply_text(
                parsed_or_err + MSG_FLOW_NO_MENU,
                parse_mode="HTML",
                reply_markup=HIDDEN_MENU,
            )
            return
        flow[uid] = {"stage": "proof", "static_line": parsed_or_err}
        await msg.reply_text(
            f"Принято: <b>{html_escape(parsed_or_err)}</b>\n\n"
    ...
    if st.get("stage") == "proof":
        ...
        sid = add_submission(
            ...
            server_name=parse_static_and_server(static_line)[2],
        )
        flow[uid] = {"stage": "idle", "static_line": None}
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("❌ Отклонить", callback_data=f"rjask_{sid}"),
            ],
            [
                InlineKeyboardButton("💸 Оплачено", callback_data=f"pay_{sid}"),
                InlineKeyboardButton("🧾 Не оплачено", callback_data=f"unpay_{sid}"),
            ],
        ]
    )
...
async def send_submission_proof(chat_id: int, context: ContextTypes.DEFAULT_TYPE, item: dict) -> None:
    kind = str(item.get("proof_kind", ""))
    ref = str(item.get("proof_ref", ""))
    if not ref:
        return
    try:
        if kind == "photo":
            await context.bot.send_photo(chat_id=chat_id, photo=ref)
        elif kind == "video":
            await context.bot.send_video(chat_id=chat_id, video=ref)
        elif kind == "video_note":
            await context.bot.send_video_note(chat_id=chat_id, video_note=ref)
        elif kind == "document":
            await context.bot.send_document(chat_id=chat_id, document=ref)
        elif kind == "image":
            await context.bot.send_document(chat_id=chat_id, document=ref)
        elif kind == "url":
            await context.bot.send_message(chat_id=chat_id, text=f"Ссылка на видео:\n{ref}")
        else:
            await context.bot.send_message(chat_id=chat_id, text=f"Доказательство ({kind}): {ref}")
    except Exception as e:
        log.warning("send submission proof to %s failed: %s", chat_id, e)
...
async def callback_moderate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("ap_"):
        ...
        if item.get("status") != "pending":
            ...
            return
        admin_pending_reject_reason[q.from_user.id] = sid
        await q.answer("Ожидаю причину")
        await q.message.reply_text(
            f"Напиши причину отказа для заявки #{sid} следующим сообщением."
        )
        return
        ...
        if not item:
            await q.answer("Заявка не найдена", show_alert=True)
            return
        if set_submission_payment(sid, "paid"):
            await q.answer("Помечено как оплачено")
            try:
                await q.message.reply_text(f"Заявка #{sid} помечена как оплаченная 💸")
            except Exception:
                pass
        else:
        ...
        if not item:
            await q.answer("Заявка не найдена", show_alert=True)
            return
        if set_submission_payment(sid, "unpaid"):
            await q.answer("Помечено как не оплачено")
            try:
                await q.message.reply_text(f"Заявка #{sid} помечена как не оплаченная")
            except Exception:
                pass
        else:
            await q.answer("Не удалось")
...
async def handle_admin_menu_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in admin_pending_reject_reason:
        sid = admin_pending_reject_reason.pop(uid)
        reason = text.strip()
        if not reason:
            await update.message.reply_text("Причина пустая. Отмена.")
            return
        item = get_submission(sid)
        if not item:
        ...
        if item.get("status") != "pending":
            ...
            return
        set_submission_reject_reason(sid, reason)
        set_submission_status(sid, "rejected")
        await update.message.reply_text(f"Заявка #{sid} отклонена с причиной.")
        try:
            await context.bot.send_message(
                ...
                text=(
                    "Заявка отклонена ❌\n"
                    f"Причина: {reason}\n\n"
                    "Исправь и отправь заново через /start."
                ),
            )
        except Exception as e:
            log.warning("notify user reject reason %s: %s", item.get("user_id"), e)
        return

    if text == "◀️ Закрыть админку":
        admin_pending.pop(uid, None)
        admin_pending_reject_reason.pop(uid, None)
    ...
    if text == "📊 Статистика":
        ...
        registry = _load_json(SUBMITTED_REGISTRY_FILE, {"items": []}).get("items", [])
        ...
        paid = sum(1 for x in items if x.get("payment_status") == "paid")
        await update.message.reply_text(
            f"👥 Пользователи: <b>{len(users)}</b>\n"
            f"📨 Отправлено заявок (реестр): <b>{len(registry)}</b>\n"
    ...
    if text == "📋 Заявки на модерацию":
        ...
        for it in pend[-10:]:
            ...
            payment_status = str(it.get("payment_status", "unpaid"))
            reject_reason = str(it.get("reject_reason", "")).strip()
            preview = (
                ...
                f"Статус оплаты: <b>{'оплачено' if payment_status == 'paid' else 'не оплачено'}</b>"
            )
            if reject_reason:
                preview += f"\nПричина отказа: <i>{html_escape(reject_reason)}</i>"
            await update.message.reply_text(
                preview, parse_mode="HTML", reply_markup=kb
            )
            await send_submission_proof(update.effective_chat.id, context, it)
        return
...
def main() -> None:
    ...
    if not SERVER_ALIASES:
        log.warning("Список серверов пуст (servers.txt) — проверка сервера не будет работать")
    ...
    app.add_handler(
        CallbackQueryHandler(
            callback_moderate,
            pattern=r"^(ap|rj|rjask|pay|unpay)_\d+$",
        )
    )
